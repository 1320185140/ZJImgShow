...
+ (CGSize)judgeImageFrame:(CGSize)ImgSize{
    float img_width = ImgSize.width;
    float img_Height = ImgSize.height;
    float result_width = 0;
    float result_heigt = 0;
    float max_width = SCREEN_WIDTH - 130;
    float max_height = SCREEN_WIDTH -130;
    float min_width = (SCREEN_WIDTH - 130)/2;
    float min_height = (SCREEN_WIDTH - 130)/2;
    if(img_Height == 0 || img_width == 0){
        return CGSizeMake(0, 0);
    }
    if (img_Height/img_width > 2.5) {
        result_heigt = max_height;
        result_width = result_heigt * (img_width/img_Height);
        result_width = (result_width > min_width)?result_width:min_width;
    }else if(img_width/img_Height > 2.5){
        result_width = max_width;
        result_heigt = result_width * (img_Height/img_width);
        result_heigt = (result_heigt >min_height)?result_heigt:min_height;
    }else{
     ...
     return CGSizeMake(result_width, result_heigt);
}
